...
This repository contains the code for our paper. We propose a principled and effective preference optimization algorithm, AMaPO, to address the **Overfitting-Underfitting Dilemma** in DPO-style algorithms, where existing margin designs (i.e. DPO, SimPO) apply excessive, wasteful gradients to correctly ranked samples (overfitting) while providing insufficient corrective signals for misranked ones (underfitting). AMaPO employs an instance-wise adaptive margin refined by scaling function and estimated oracle margin. Extensive experiments demonstrate that AMaPO outperforms DPO and its latest variants (i.e. alpha-DPO) across **Preference Ranking** and **Alignment Downstream Task** benchmarks under various settings without external tuning hyperparameters.
